  ...
  async GET(req, ctx) {
    ...
    for await (const entry of db.list({ prefix: ["pledges"] })) {
      ...
      if (
        ...
        !pledge.phone.toLowerCase().includes(
          filters.search.toLowerCase(),
        ) &&
    ...
    // Group pledges by phone
    const groupedPledges = new Map<string, Pledge[]>();
    pledges.forEach((pledge) => {
      const existing = groupedPledges.get(pledge.phone) || [];
      existing.push(pledge);
      groupedPledges.set(pledge.phone, existing);
    });
